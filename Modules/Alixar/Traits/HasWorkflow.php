...
namespace Modules\Alixar\Traits;
 ...
 * draft -> validated lifecycles (typical ERP functionality).
 * 
 * Re-implemented locally in Alixar after being extracted from Alxarafe core.
 */
trait HasWorkflow
{
    /**
            ...
            foreach ($definition['transitions'] as $name => $transition) {
                if (in_array($currentState, $transition['from'], true)) {
                    ...
                    if (class_exists('\Alxarafe\Lib\Trans')) {
                        $label = \Alxarafe\Lib\Trans::_('transition_' . $name);
