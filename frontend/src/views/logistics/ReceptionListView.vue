...
const columns: Column[] = [
  ...
  { key: 'dateDelivery', label: 'Recepción' },
]

const loadData = async () => {
  try {
    loading.value = true
    rows.value = await api.getReceptions({ limit: limit.value, page: page.value - 1 })
...
<template>
  <div>
    ...
    <EntityList v-else :columns="columns" :rows="rows" :total="total" :page="page" :limit="limit"
      title="Recepciones" create-label="Nueva Recepción" detail-route="reception-ficha"
      @create="router.push({ name: 'resource-create', params: { resource: 'recepciones' } })" @page-change="(p: number) => { page = p; loadData() }" @limit-change="(l: number) => { limit = l; page = 1; loadData() }" />
  </div>
</template>
