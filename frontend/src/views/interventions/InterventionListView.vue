...
const columns: Column[] = [
  ...
  { key: 'thirdPartyId', label: 'Tercero ID' },
  { key: 'dateStart', label: 'Inicio' },
]

const loadData = async () => {
  try {
    loading.value = true
    rows.value = await api.getInterventions({ limit: limit.value, page: page.value - 1 })
...
<template>
  <div>
    ...
    <EntityList v-else :columns="columns" :rows="rows" :total="total" :page="page" :limit="limit"
      title="Partes de Trabajo / Intervenciones" create-label="Nuevo Parte" detail-route="intervention-ficha"
      @create="router.push({ name: 'resource-create', params: { resource: 'intervenciones' } })" @page-change="(p: number) => { page = p; loadData() }" @limit-change="(l: number) => { limit = l; page = 1; loadData() }" />
  </div>
</template>
