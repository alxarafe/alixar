...
onMounted(async () => {
  try {
    entity.value = await api.getIntervention(Number(route.params.id))
  } catch (e) {
    console.error(e)
  } finally {
    loading.value = false
...
<template>
  ...
  <div v-else-if="entity" class="detail-layout">
    <div class="sidebar-detail glass-panel">
      <div class="detail-header"><h2><router-link to="/intervenciones" class="back-btn-title">⬅️</router-link> Parte {{ entity.ref }}</h2></div>
      <nav class="detail-nav"><router-link :to="{ name: 'intervention-ficha' }" active-class="active">Ficha</router-link></nav>
    </div>
    <div class="main-detail glass-panel">
      <router-view :entity="entity" schemaName="intervenciones"></router-view>
    </div>
  </div>
</template>
