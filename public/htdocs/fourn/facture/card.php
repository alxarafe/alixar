...
if (empty($reshook)) {
    $backurlforlist = constant('BASE_URL') . '/fourn/facture/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = constant('BASE_URL') . '/fourn/facture/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'settransportmode' && ($user->hasRight("fournisseur", "facture", "creer") || $user->hasRight("supplier_invoice", "creer"))) {
        ...
        if (GETPOST('type') == FactureFournisseur::TYPE_CREDIT_NOTE) {
            ...
            if (!$error) {
                ...
                if (!$error && GETPOST('origin', 'alpha') && GETPOST('originid')) {
                    ...
                    require_once constant('DOL_DOCUMENT_ROOT') . '/' . $element . '/class/' . $subelement . '.class.php';
        ...
        if (
            $object->status == FactureFournisseur::STATUS_CLOSED
...
if ($action == 'create') {
    ...
    if ($id > 0 || !empty($ref)) {
        ...
        $linkback = '<a href="' . constant('BASE_URL') . '/fourn/facture/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if (empty($reshook)) {
            ...
            if ($object->fk_fac_rec_source > 0) {
                ...
                if ($result > 0) {
                    print ' <span class="opacitymediumbycolor paddingleft">';
                    $link = '<a href="' . constant('BASE_URL') . '/fourn/facture/card-rec.php?facid=' . $tmptemplate->id . '">' . dol_escape_htmltag($tmptemplate->title) . '</a>';
            ...
            if (isModEnabled('incoterm')) {
                ...
                if ($usercancreate) {
                    print '<a class="editfielda" href="' . constant('BASE_URL') . '/fourn/facture/card.php?facid=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
                } else {
                    print '&nbsp;';
                }
                print '</td></tr></table>';
        ...
        if ($action != 'presend') {
            ...
            if (empty($reshook)) {
                ...
                if ($object->type != FactureFournisseur::TYPE_CREDIT_NOTE && $object->status == FactureFournisseur::STATUS_VALIDATED && $object->paid == 0) {
                    print '<a class="butAction' . ($conf->use_javascript_ajax ? ' reposition' : '') . '" href="' . constant('BASE_URL') . '/fourn/facture/paiement.php?facid=' . $object->id . '&amp;action=create' . ($object->fk_account > 0 ? '&amp;accountid=' . $object->fk_account : '') . '">' . $langs->trans('DoPayment') . '</a>'; // must use facid because id is for payment id not invoice
                }
                ...
                if ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE || $object->type == FactureFournisseur::TYPE_DEPOSIT || $object->type == FactureFournisseur::TYPE_STANDARD) {
                    ...
                    if ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE && $object->status == 1 && $object->paid == 0) {
                        if ($resteapayer == 0) {
                            ...
                            print '<a class="butAction" href="' . constant('BASE_URL') . '/fourn/facture/paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid=' . $object->fk_account . '">' . $langs->trans('DoPaymentBack') . '</a>';
                        }
                    }
                    ...
                    // For credit note
                    if (
                        $object->type == FactureFournisseur::TYPE_CREDIT_NOTE && $object->status == 1 && $object->paid == 0 && $usercancreate
