    ...
    if ((isModEnabled("fournisseur") && $user->hasRight("fournisseur", "facture", "lire") && !getDolGlobalString('MAIN_USE_NEW_SUPPLIERMOD')) || (isModEnabled("supplier_invoice") && $user->hasRight("supplier_invoice", "lire"))) {
        ...
        if ($resql) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                // Author
                print '<td class="nowrap" width="50"><a href="' . constant('BASE_URL') . '/user/card.php?id=' . $objf->userid . '">' . img_object($langs->trans("ShowUser"), 'user') . ' ' . $objf->login . '</a></td>';
                ...
                if ($resqlp) {
                    ...
                    while ($j < $nump) {
                        ...
                        // Auteur
                        print '<td class="nowrap" width="50"><a href="' . constant('BASE_URL') . '/user/card.php?id=' . $objp->userid . '">' . img_object($langs->trans("ShowUser"), 'user') . ' ' . $objp->login . '</a></td>';
