    ...
    if ($action != 'reconcile') {
        if ($object->canBeConciliated() > 0) {
            ...
            if ($user->hasRight('banque', 'consolidate')) {
                ...
                $buttonreconcile = '<a class="butAction" style="margin-bottom: 5px !important; margin-top: 5px !important" href="' . constant('BASE_URL') . '/compta/bank/bankentries_list.php?action=reconcile&sortfield=b.datev,b.dateo,b.rowid&sortorder=asc,asc,asc&search_conciliated=0' . $newparam . '">' . $titletoconciliatemanual . '</a>';
            } else {
    ...
    */
    ...
    while ($i < $imaxinloop) {
        ...
        if ($objp->conciliated && $bankaccount->canBeConciliated() > 0) {  // If line not conciliated and account can be conciliated
            print '<a class="editfielda" href="' . constant('BASE_URL') . '/compta/bank/line.php?save_lastsearch_values=1&rowid=' . $objp->rowid . ($object->id > 0 ? '&account=' . $object->id : '') . '&page=' . $page . '">';
            print img_edit();
            print '</a>';
        } else {
