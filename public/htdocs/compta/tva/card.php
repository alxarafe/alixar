...
if ($id > 0) {
    ...
    if ($resql) {
        ...
        if ($num > 0) {
            ...
            while ($i < $num) {
                ...
                print '<a href="' . constant('BASE_URL') . '/compta/payment_vat/card.php?id=' . $objp->rowid . '">' . img_object($langs->trans("Payment"), "payment") . ' ' . $objp->rowid . '</a>';
                print '</td>';
    ...
    if ($action != 'edit') {
        ...
        if ($object->paye && $user->hasRight('tax', 'charges', 'creer')) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/compta/tva/card.php?id=' . $object->id . '&action=reopen&token=' . newToken() . '">' . $langs->trans("ReOpen") . "</a></div>";
        }

        // Edit
        if ($object->paye == 0 && $user->hasRight('tax', 'charges', 'creer')) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/compta/tva/card.php?id=' . $object->id . '&action=edit&token=' . newToken() . '">' . $langs->trans("Modify") . "</a></div>";
        }
        ...
        if ($object->paye == 0 && ((price2num($object->amount) < 0 && price2num($resteapayer, 'MT') < 0) || (price2num($object->amount) > 0 && price2num($resteapayer, 'MT') > 0)) && $user->hasRight('tax', 'charges', 'creer')) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/compta/paiement_vat.php?id=' . $object->id . '&action=create&token=' . newToken() . '">' . $langs->trans("DoPayment") . '</a></div>';
        }
        ...
        if (
            ...
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/compta/tva/card.php?id=' . $object->id . '&token=' . newToken() . '&action=paid">' . $langs->trans("ClassifyPaid") . "</a></div>";
        }

        // Clone
        if ($user->hasRight('tax', 'charges', 'creer')) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/compta/tva/card.php?id=' . $object->id . '&token=' . newToken() . '&action=clone">' . $langs->trans("ToClone") . "</a></div>";
        }
