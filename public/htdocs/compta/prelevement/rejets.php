...
if ($result) {
    ...
    if ($num) {
        ...
        while ($i < $maxlim) {
            ...
            print '<a href="' . constant('BASE_URL') . 'compta/prelevement/line.php?id=' . $obj->rowid . '">';
            print substr('000000' . $obj->rowid, -6) . "</a></td>";

            if ($bon->checkIfSalaryBonPrelevement()) {
                print '<td><a href="' . constant('BASE_URL') . 'salaries/card.php?id=' . $obj->socid . '">' . $obj->name . "</a></td>\n";
            } else {
                $thirdpartystatic->id = $obj->socid;
                $thirdpartystatic->name = $obj->name;

                print '<td class="tdoverlowmax200"><a href="' . constant('BASE_URL') . 'comm/card.php?socid=' . $obj->socid . '">' . $thirdpartystatic->getNomUrl(1) . "</a></td>\n";
            }
