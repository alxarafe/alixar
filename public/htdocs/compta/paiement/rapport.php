...
require_once constant('DOL_DOCUMENT_ROOT') . '/core/modules/rapport/pdf_paiement.class.php';
...
if ($year) {
    if (is_dir($dir . '/' . $year)) {
        ...
        foreach ($files as $f) {
            ...
            print '<td><a data-ajax="false" href="' . constant('BASE_URL') . 'document.php?modulepart=facture_paiement&amp;file=' . urlencode($relativepath) . '">' . img_pdf() . ' ' . $f['name'] . '</a>' . $formfile->showPreview($f['name'], 'facture_paiement', $relativepath, 0) . '</td>';
