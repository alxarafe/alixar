...
if (empty($reshook)) {
    ...
    $backurlforlist = constant('BASE_URL') . '/knowledgemanagement/knowledgerecord_list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = constant('BASE_URL') . '/knowledgemanagement/knowledgerecord_card.php?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . constant('BASE_URL') . '/knowledgemanagement/knowledgerecord_list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
