    ...
    if ($num) {
        ...
        while ($i < $num) {
            ...
            if ($obj->commande_fournisseur_id) {
                print '<a href="' . constant('BASE_URL') . 'commande_fournisseur/card.php?id=' . $obj->commande_fournisseur_id . '">' . $obj->commande_fournisseur_ref . '</a>';
            }
            print '</td></tr>';
            $i++;
        }
...
if ($resql) {
    ...
    if ($num) {
        ...
        print ' <a href="' . constant('BASE_URL') . 'reception/list.php?search_status=1" alt="' . $langs->trans("GoOnList") . '"><span class="badge">' . $num . '</span></a>';
        while ($i < $num) {
            $obj = $db->fetch_object($resql);
