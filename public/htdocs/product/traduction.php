...
if (empty($reshook)) {
    if ($action == '') {
        if ($user->hasRight('produit', 'creer') || $user->hasRight('service', 'creer')) {
            print '<a class="butAction" href="' . constant('BASE_URL') . '/product/traduction.php?action=add&token=' . newToken() . '&id=' . $object->id . '">' . $langs->trans("Add") . '</a>';
            if ($cnt_trans > 0) {
                print '<a class="butAction" href="' . constant('BASE_URL') . '/product/traduction.php?action=edit&token=' . newToken() . '&id=' . $object->id . '">' . $langs->trans("Modify") . '</a>';
            }
        }
    }
}
...
if ($action == 'add' && ($user->hasRight('produit', 'creer') || $user->hasRight('service', 'creer'))) {
    ...
    print FormAdmin::selectLanguage(GETPOST('forcelangprod'), 'forcelangprod', 0, $object->multilangs, 1);
    print '</td></tr>';
