...
// Object card
// ------------------------------------------------------------
$linkback = '<a href="' . constant('BASE_URL') . '/product/inventory/list.php">' . $langs->trans("BackToList") . '</a>';
...
if ($resql) {
    ...
    while ($i < $num) {
        ...
        if ($object->status == $object::STATUS_DRAFT || $object->status == $object::STATUS_VALIDATED) {
            ...
            // Picto delete line
            print '<td class="right">';
            print '<a class="reposition" href="' . constant('BASE_URL') . '/product/inventory/inventory.php?id=' . $object->id . '&lineid=' . $obj->rowid . '&action=deleteline&page=' . $page . $paramwithsearch . '&token=' . newToken() . '">' . img_delete() . '</a>';
