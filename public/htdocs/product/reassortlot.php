...
while ($i < $imaxinloop) {
    ...
    print '<a href="' . constant('BASE_URL') . 'product/stock/movement_list.php?idproduct=' . $product_static->id . '&search_warehouse=' . $objp->fk_entrepot . '&search_batch=' . ($objp->batch != 'Undefined' ? $objp->batch : 'Undefined') . '">' . $langs->trans("Movements") . '</a>';
    print '</td>';
    if (!$i) {
        $totalarray['nbfield']++;
    }
