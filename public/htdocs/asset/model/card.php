...
if (empty($reshook)) {
    ...
    $backurlforlist = constant('BASE_URL') . '/asset/model/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = '/asset/model/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
...
$help_url = '';
ViewMain::llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-asset page-model-card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    //  $MAXEVENT = 10;
    //
    //  $morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', constant('BASE_URL') . '/asset/model/agenda.php?id=' . $object->id);
    //
