...
if (!empty($object->id)) {
    ...
    $btnstatus = $object->status < Ticket::STATUS_CLOSED && $action != "presend" && $action != "presend_addmessage";
    $url = constant('BASE_URL') . '/comm/action/card.php?action=create&datep=now&origin=ticket&originid=' . $object->id . '&projectid=' . $object->fk_project . '&backtopage=' . $_SERVER["PHP_SELF"] . ('?track_id=' . $object->track_id);
