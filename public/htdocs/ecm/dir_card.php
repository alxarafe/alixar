...
if ($module == 'ecm') {
    ...
    $morehtmlref = '<a href="' . constant('BASE_URL') . '/ecm/index.php">' . $langs->trans("ECMRoot") . '</a> -> ' . $s;
}
if ($module == 'medias') {
    $s = 'medias -> ';
    $result = 1;
...
if ($action != 'edit' && $action != 'delete' && $action != 'deletefile') {
    ...
    if ($permissiontoadd) {
        print '<a class="butAction" href="' . constant('BASE_URL') . '/ecm/dir_add_card.php?action=create&token=' . newToken() . ($module ? '&module=' . $module : '') . '&catParent=' . $section . '">' . $langs->trans('ECMAddSection') . '</a>';
    } else {
