...
if ($action == 'create') {
    ...
    if ($result > 0) {
        ...
        if ($action == 'edit' && ($object->status < 3 || $object->status == 99)) {
            ...
            $linkback = '<a href="' . constant('BASE_URL') . '/expensereport/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    */
    ...
    if ($user->hasRight('expensereport', 'to_paid') && isModEnabled("bank") && $object->status == ExpenseReport::STATUS_APPROVED) {
        ...
        if ($remaintopay == 0) {
            ...
            print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/expensereport/payment/payment.php?id=' . $object->id . '&amp;action=create">' . $langs->trans('DoPayment') . '</a></div>';
        }
    }
