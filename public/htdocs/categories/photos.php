...
if ($object->id) {
    ...
    $morehtmlref = '<br><div class="refidno"><a href="' . constant('BASE_URL') . '/categories/index.php?leftmenu=cat&type=' . $type . '">' . $langs->trans("Root") . '</a> >> ';
    ...
    if ($action != 'ajout_photo') {
        ...
        if (is_array($listofphoto) && count($listofphoto)) {
            ...
            foreach ($listofphoto as $key => $obj) {
                ...
                print '<a href="' . constant('BASE_URL') . '/viewimage.php?modulepart=category&entity=' . $object->entity . '&file=' . urlencode($pdir . $obj['photo']) . '" alt="Original size" target="_blank" rel="noopener noreferrer">';
