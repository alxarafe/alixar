                    ...
                    if ($objp->fk_product > 0 && ($type == Product::TYPE_PRODUCT || getDolGlobalString('STOCK_SUPPORTS_SERVICES')) && isModEnabled('stock')) {
                        ...
                        if ($product->stock_reel < $toBeShipped[$objp->fk_product]) {
                            ...
                            if (getDolGlobalString('STOCK_CORRECT_STOCK_IN_SHIPMENT')) {
                                ...
                                print ' &nbsp; ' . $langs->trans("GoTo") . ' <a href="' . constant('BASE_URL') . '/product/stock/product.php?id=' . ((int) $product->id) . '&action=correction&token=' . newToken() . '&nbpiece=' . urlencode((string) ($nbPiece)) . '&backtopage=' . urlencode((string) ($_SERVER["PHP_SELF"] . '?id=' . ((int) $object->id))) . '">' . $langs->trans("CorrectStock") . '</a>';
                            }
                        }
                        print '</td>';
        ...
        if (empty($user->socid)) {
            ...
            if (!isModEnabled('stock') && ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED)) {
                if ($user->hasRight('expedition', 'creer')) {
                    print '<a class="butAction" href="' . constant('BASE_URL') . '/expedition/card.php?action=create&amp;origin=commande&amp;object_id=' . $id . '">' . $langs->trans("CreateShipment") . '</a>';
            ...
            if ($user->hasRight('expedition', 'creer')) {
                ...
                if (isModEnabled('stock')) {
                    ...
                    if (count($formproduct->cache_warehouses) <= 0) {
                        print ' &nbsp; ' . $langs->trans("WarehouseSourceNotDefined") . ' <a href="' . constant('BASE_URL') . '/product/stock/card.php?action=create">' . $langs->trans("AddOne") . '</a>';
                    }
                    //print '</td>';
                }
                //print '<td class="center">';
