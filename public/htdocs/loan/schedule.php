    ...
    foreach ($echeances->lines as $line) {
        ...
        if (!empty($line->fk_bank)) {
            ...
            if (!empty($line->fk_payment_loan)) {
                print '&nbsp;<a href="' . constant('BASE_URL') . '/loan/payment/card.php?id=' . $line->fk_payment_loan . '">(' . img_object($langs->trans("Payment"), "payment") . ' ' . $line->fk_payment_loan . ')</a>';
            }
        } elseif (!$printed) {
            print '<a class="butAction smallpaddingimp" href="' . constant('BASE_URL') . '/loan/payment/payment.php?id=' . $object->id . '&action=create">' . $langs->trans('DoPayment') . '</a>';
            $printed = true;
        }
        print '</td>';
        print '</tr>' . "\n";
