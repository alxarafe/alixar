...
if (empty($reshook)) {
    $backurlforlist = constant('BASE_URL') . '/don/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = constant('BASE_URL') . '/don/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
...
if (!empty($id) && $action != 'edit') {
    ...
    if ($resql) {
        ...
        while ($i < $num) {
            ...
            print '<a href="' . constant('BASE_URL') . '/don/payment/card.php?id=' . $objp->rowid . '">' . img_object($langs->trans("Payment"), "payment") . ' ' . $objp->rowid . '</a></td>';
    ...
    if (empty($reshook)) {
        ...
        if ($object->statut == $object::STATUS_VALIDATED && $object->paid == 0 && $user->hasRight('don', 'creer')) {
            if ($remaintopay == 0) {
                ...
                print '<div class="inline-block divButAction"><a class="butAction" href="' . constant('BASE_URL') . '/don/payment/payment.php?rowid=' . $object->id . '&action=create&token=' . newToken() . '">' . $langs->trans('DoPayment') . '</a></div>';
            }
        }
