...
?>
    ...
    th a {
        font-weight: <?php echo($useboldtitle ? 'bold' : 'normal'); ?> !important;
    }
    ...
    th.liste_titre a div div:hover, th.liste_titre_sel a div div:hover {
        text-decoration: underline;
    }

    tr.liste_titre th.liste_titre_sel:not(.maxwidthsearch), tr.liste_titre td.liste_titre_sel:not(.maxwidthsearch),
    tr.liste_titre th.liste_titre:not(.maxwidthsearch), tr.liste_titre td.liste_titre:not(.maxwidthsearch) {
        opacity: 0.8;
    }

    /* th.liste_titre_sel a, th.liste_titre a, td.liste_titre_sel a, td.liste_titre a { color: #766; } */
    tr.liste_titre_filter th.liste_titre {
        text-align: unset;
    }
...
*/
    ...
    <?php if (!getDolGlobalString('THEME_ADD_BACKGROUND_ON_INPUT')) { ?> border <?php echo !getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT') ? '-bottom' : ''; ?>: solid 1px var(--inputbordercolor);
    <?php } ?>
    }
    ...
    div.tabBar input, div.tabBar input.flat, div.tabBar textarea, div.tabBar textarea.flat, div.tabBar form.flat select, div.tabBar select, div.tabBar select.flat, div.tabBar .dataTables_length label select {
        border <?php echo !getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT') ? '-bottom' : ''; ?>: solid 1px var(--inputbordercolor);
    <?php
    if (getDolGlobalString('THEME_ADD_BACKGROUND_ON_INPUT')) { ?> background-color: #f8f8fa;
...
*/
    ...
    input[type=file] {
        background-color: transparent;
        box-shadow: none;
    <?php if (!getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT')) { ?> border-top: none;
        border-left: none;
        border-right: none;
    <?php } ?> border <?php echo !getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT') ? '-bottom' : ''; ?>: solid 1px var(--inputbordercolor);
    }
    ...
    input[name=price], input[name=weight], input[name=volume], input[name=surface], input[name=sizeheight], input[name=net_measure], select[name=incoterm_id] {
        margin-right: 6px;
    }
    ...
    .fa-toggle-on, .fa-toggle-off, .size2x {
        font-size: 2em;
    }
...
if ($conf->browser->layout == 'phone') {
    ...
    <?php if ($conf->browser->name == 'ie') { ?> min-width: 150px;
        width: 100%;
        display: inline;
    <?php } else { ?> flex-grow: 1;
        flex-shrink: 0;
    <?php } ?>
    }
    ...
    @media only screen and (max-width: 570px) {
        ...
        .maxwidthonsmartphone, #search_newcompany.ui-autocomplete-input {
            max-width: 100px;
        }
    ...
    */
    ...
    #id-left {
        padding-top: 20px;
        padding-bottom: 5px;
    <?php if (getDolGlobalString('MAIN_USE_TOP_MENU_SEARCH_DROPDOWN')) { ?> padding-top: 18px;
    <?php } ?>
    }
...
div.statusref {
    ...
    div.topmenuimage {
    <?php if ($disableimages) { ?> display: none;
    <?php } ?>
    }
    ...
    <?php print $minwidthtmenu ? 'min-width: ' . $minwidthtmenu . 'px;' : ''; ?> text-align: center;
        vertical-align: bottom;
    <?php if (!getDolGlobalString('MAIN_MENU_INVERT')) { ?> float: <?php print $left; ?>;
    ...
    <?php } ?><?php if ($disableimages) { ?> height: 26px;
    <?php } else { ?> height: <?php print $heightmenu; ?>px;
    <?php } ?>
    }
    ...
    div.tmenucenter {
        ...
        padding-right: 2px;
        color: var(--colortextbackhmenu);
    <?php if ($disableimages) { ?> padding-top: 8px;
        height: 26px;
    <?php } else { ?> padding-top: 2px;
...
?> -webkit-box-shadow: 0 2px 23px 2px rgba(0, 0, 0, 0.2), 0 2px 6px rgba(60, 60, 60, 0.15);
    ...
    span.vsmenudisabledmargin, font.vsmenudisabledmargin {
        margin: 1px 1px 1px 6px;
    }
    ...
    */
    ...
    div.blockvmenuhelp {
    <?php if (empty($conf->dol_optimize_smallscreen)) { ?> font-family: <?php print $fontlist ?>;
        color: #000000;
        text-align: center;
        text-decoration: none;
        padding-left: 0px;
...
div.tabsElem a.tabactive::before, div.tabsElem a.tabunactive::before {
    ...
    */
    ...
    <?php if ($colorbackhmenu1 == '255,255,255') { ?> border-bottom: 3px solid var(--colortextbackhmenu) !important;
    <?php } else { ?> border-bottom: 3px solid var(--colorbackhmenu1) !important;
    <?php } ?>
    }
...
*/
    ...
    <?php
    if ($userborderontable) { ?> border-left: 1px solid var(--colortopbordertitle1);
...
*/
    ...
    .tagtable, .table-border {
        display: table;
    }

    .tagtr, .table-border-row {
        display: table-row;
    }

    .tagtd, .table-border-col, .table-key-border-col, .table-val-border-col {
        display: table-cell;
    }
...
div.pagination li:not(.paginationbeforearrows,.paginationafterarrows,.title-button) span:focus {
  ...
  box-shadow: 0px 0px 6px 1px rgba(50, 50, 50, 0.4), 0px 0px 0px rgba(60,60,60,0.1);
    ...
    table.nobottomiftotal tr.liste_total td {
        background-color: var(--inputbackgroundcolor);
    <?php if (!$userborderontable) { ?> border-bottom: 0px !important;
    <?php } ?>
    }
    ...
    div.liste_titre_bydiv {
        ...
        border-top-color: var(--colortopbordertitle1);
        border-top-style: solid;
    <?php if ($userborderontable) { ?> border-left: <?php echo $borderwidth ?>px solid var(--colortopbordertitle1);
        border-right: <?php echo $borderwidth ?>px solid var(--colortopbordertitle1);
    <?php } ?> border-collapse: collapse;
 ...
 */
    ...
    .fieldrequired {
        font-weight: bold;
        color: var(--fieldrequiredcolor) !important;
    }

    td.widthpictotitle, .table-fiche-title img.widthpictotitle {
        width: 38px;
        text-align: <?php echo $left; ?>;
    }
    ...
    div.dolgraph div.legend, div.dolgraph div.legend div {
    ...
    #tablesubscribe tr td {
        font-size: 1.15em;
    }

    #tablesubscribe .price-registration {
        font-size: 1.5em;
    }
...
?>
    ...
    .dpButtons:Active, .tpButtons:Active {
    ...
    .cal_peruser {
        padding-top: 0 !important;
        padding-bottom: 0 !important;
        padding- <?php print $left; ?>: 1px !important;
        padding- <?php print $right; ?>: 1px !important;
    }
    ...
    .treeview ul {
        ...
        margin-top: 0 !important; /* margin-bottom: 4px !important; padding-top: 2px !important; */
    }
    ...
    <?php print(preg_match('/chrome/', $conf->browser->name) ? 'padding-top: 1px;' : ''); ?>   } */
    .ui-button {
        margin-left: -2px;
    }
...
*/
    ...
    <?php if (getDolGlobalString('MAIN_JQUERY_JNOTIFY_BOTTOM')) { ?> top: auto !important;
...
.ui-corner-all { white-space: nowrap; } */
    ...
    span.select2.select2-container.select2-container--default {
        text-align: initial;
    <?php if (!getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT')) { ?> border-left: none;
        border-top: none;
        border-right: none;
    <?php } ?>
    }
    ...
    .dropdown dd ul {
        ...
        display: none;
    <?php echo $right; ?>: 0 px; /* pop is align on right */
        padding: 0 0 0 0;
        position: absolute;
        top: 2px;
        list-style: none;
    ...
    @media only screen and (max-width: 768px) {
        .imgopensurveywizard, .imgautosize {
            width: 95%;
            height: auto;
        }
    ...
    <?php echo $nbtopmenuentries ?>   menu entries) */
