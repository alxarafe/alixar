...
if ($action == 'updateMask') {
    ...
    if (!empty($maskconst) && str_ends_with($maskconst, '_MASK')) {
    ...
    if ($classname !== '') {
        ...
        if ($module->write_file($exp, $langs) > 0) {
            header("Location: " . constant('BASE_URL') . "/document.php?modulepart=expedition&file=SPECIMEN.pdf");
            return;
        } else {
...
ViewMain::llxHeader("", $langs->trans("SendingsSetup"), '', '', 0, 0, '', '', '', 'mod-admin page-expedition');
