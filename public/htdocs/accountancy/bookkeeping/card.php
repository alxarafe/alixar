...
if ($action == 'create') {
    ...
    */
    ...
    if (!empty($object->piece_num)) {
        $backlink = '<a href="' . constant('BASE_URL') . '/accountancy/bookkeeping/list.php?restore_lastsearch_values=1">' . $langs->trans('BackToList') . '</a>';
        ...
        if ($result < 0) {
            ...
            if (count($object->linesmvt) > 0) {
                ...
                if ($mode == '_tmp' && $action == '' && $permissiontoadd) {
                    ...
                    print ' &nbsp; ';
                    print '<a class="button button-cancel" href="' . constant('BASE_URL') . '/accountancy/bookkeeping/list.php">' . $langs->trans("Cancel") . '</a>';
