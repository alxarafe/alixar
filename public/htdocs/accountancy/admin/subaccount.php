...
if ($resql) {
    ...
    while ($i < min($num, $limit)) {
        ...
        if (!empty($arrayfields['type']['checked'])) {
            ...
            // Customer
            if ($obj->type == 1) {
                $s .= '<a class="customer-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Customer") . '" href="' . constant('BASE_URL') . 'comm/card.php?socid=' . $obj->rowid . '">' . $langs->trans("Customer") . '</a>';
            } elseif ($obj->type == 2) {
                // Supplier
                $s .= '<a class="vendor-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Supplier") . '" href="' . constant('BASE_URL') . 'fourn/card.php?socid=' . $obj->rowid . '">' . $langs->trans("Supplier") . '</a>';
            } elseif ($obj->type == 3) {
                // User - Employee
                $s .= '<a class="user-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Employee") . '" href="' . constant('BASE_URL') . 'user/card.php?id=' . $obj->rowid . '">' . $langs->trans("Employee") . '</a>';
            }
            print $s;
            if ($obj->nature == 2) {
