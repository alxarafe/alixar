...
ViewMain::llxHeader('', $langs->trans("CustomersVentilation"), $help_url, '', 0, 0, '', '', '', 'mod-accountancy accountancy-customer page-index');
...
for ($i = 1; $i <= 12; $i++) {
    ...
    if (!empty($tmp['mday'])) {
        ...
        print '<a href="' . constant('BASE_URL') . '/accountancy/customer/list.php?' . $param . '">';
    }
...
for ($i = 1; $i <= 12; $i++) {
    ...
    if (!empty($tmp['mday'])) {
        $param = 'search_date_startday=1&search_date_startmonth=' . $cursormonth . '&search_date_startyear=' . $cursoryear;
        $param .= '&search_date_endday=' . $tmp['mday'] . '&search_date_endmonth=' . $tmp['mon'] . '&search_date_endyear=' . $tmp['year'];
        print '<a href="' . constant('BASE_URL') . '/accountancy/customer/lines.php?' . $param . '">';
    }
    print $langs->trans('MonthShort' . str_pad((string) $j, 2, '0', STR_PAD_LEFT));
    if (!empty($tmp['mday'])) {
        print '</a>';
...
if ($resql) {
    ...
    while ($row = $db->fetch_row($resql)) {
        ...
        if ($row[0] == 'tobind') {
            print $langs->trans("UseMenuToSetBindindManualy", constant('BASE_URL') . '/accountancy/customer/list.php?search_year=' . ((int) $y), $langs->transnoentitiesnoconv("ToBind"));
        } else {
            print $row[1];
        }
        print '</td>';
