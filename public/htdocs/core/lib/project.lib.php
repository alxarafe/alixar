            ...
            if ($showline) {
                ...
                if (count($arrayfields) > 0 && !empty($arrayfields['t.duration_effective']['checked'])) {
                    ...
                    if ($showlineingray) {
                        ...
                        print '<a href="' . constant('BASE_URL') . '/projet/tasks/time.php?id=' . $lines[$i]->id . ($showproject ? '' : '&withproject=1') . '">';
                    }
    ...
    if (
        ...
        if (count($arrayfields) > 0 && !empty($arrayfields['t.duration_effective']['checked'])) {
            ...
            if ($projectidfortotallink > 0) {
                print '<a href="' . constant('BASE_URL') . '/projet/tasks/time.php?projectid=' . $projectidfortotallink . ($showproject ? '' : '&withproject=1') . '">';
            }
            ...
            if (empty($mine) || !empty($tasksrole[$lines[$i]->id])) {
                ...
                */
                ...
                if (!empty($arrayfields['timeconsumed']['checked'])) {
                    ...
                    if ($lines[$i]->duration_effective) {
                        print '<a href="' . constant('BASE_URL') . '/projet/tasks/time.php?id=' . $lines[$i]->id . '">';
            ...
            if (empty($mine) || !empty($tasksrole[$lines[$i]->id])) {
                ...
                */
                ...
                if (!empty($arrayfields['timeconsumed']['checked'])) {
                    ...
                    // $lines[$i]->duration_effective is a denormalised field = summ of time spent by everybody for task. What we need is time consumed by user
                    if ($lines[$i]->duration_effective) {
                        print '<a href="' . constant('BASE_URL') . '/projet/tasks/time.php?id=' . ((int)$lines[$i]->id) . '">';
                        print convertSecondToTime($lines[$i]->duration_effective, 'allhourmin');
                        print '</a>';
                    } else {
                        print '--:--';
                    ...
                    $tmptimespent = $taskstatic->getSummaryOfTimeSpent($fuser->id);
                    if ($tmptimespent['total_duration']) {
                        print '<a href="' . constant('BASE_URL') . '/projet/tasks/time.php?id=' . ((int)$lines[$i]->id) . '&search_user=' . ((int)$fuser->id) . '">';
                        print convertSecondToTime($tmptimespent['total_duration'], 'allhourmin');
                        print '</a>';
                    } else {
                        print '--:--';
...
function print_projecttasks_array($db, $form, $socid, $projectsListId, $mytasks = 0, $status = -1, $listofoppstatus = array(), $hiddenfields = array(), $max = 0)
{
    ...
    if ($resql) {
        ...
        print '<tr class="liste_titre">';
        print_liste_field_titre($title . '<a href="' . constant('BASE_URL') . '/projet/list.php?search_status=' . ((int)$status) . '"><span class="badge marginleftonlyshort">' . $num . '</span></a>', $_SERVER["PHP_SELF"], "", "", "", "", $sortfield, $sortorder);
