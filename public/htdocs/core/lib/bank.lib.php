...
function bank_prepare_head(Account $object)
{
    ...
    $head[$h][0] = constant('BASE_URL') . '/compta/bank/card.php?id=' . $object->id;
    $head[$h][1] = $langs->trans("BankAccount");
    $head[$h][2] = 'bankname';
    $h++;

    $head[$h][0] = constant('BASE_URL') . "/compta/bank/bankentries_list.php?id=" . $object->id;
    ...
    //    {
    $head[$h][0] = constant('BASE_URL') . "/compta/bank/treso.php?account=" . $object->id;
    $head[$h][1] = $langs->trans("PlannedTransactions");
    $head[$h][2] = 'cash';
    $h++;
    //    }

    $head[$h][0] = constant('BASE_URL') . "/compta/bank/annuel.php?account=" . $object->id;
    $head[$h][1] = $langs->trans("IOMonthlyReporting");
    $head[$h][2] = 'annual';
    $h++;

    $head[$h][0] = constant('BASE_URL') . "/compta/bank/graph.php?account=" . $object->id;
    ...
    $head[$h][0] = constant('BASE_URL') . "/compta/bank/document.php?account=" . $object->id;
    ...
    /*$head[$h][0] = constant('BASE_URL') . "/compta/bank/info.php?id=" . $object->id;
...
function bank_admin_prepare_head($object)
{
    ...
    $head[$h][0] = constant('BASE_URL') . '/admin/bank.php';
    $head[$h][1] = $langs->trans("Miscellaneous");
    $head[$h][2] = 'general';
    $h++;

    $head[$h][0] = constant('BASE_URL') . '/admin/chequereceipts.php';
    ...
    $head[$h][0] = constant('BASE_URL') . '/admin/bank_extrafields.php';
    ...
    $head[$h][0] = constant('BASE_URL') . '/admin/bankline_extrafields.php';
...
function account_statement_prepare_head($object, $num)
{
    ...
    $head[$h][0] = constant('BASE_URL') . '/compta/bank/releve.php?account=' . $object->id . '&num=' . $num;
    ...
    $head[$h][0] = constant('BASE_URL') . "/compta/bank/account_statement_document.php?account=" . $object->id . "&num=" . $num;
    $head[$h][1] = $langs->trans("Documents");
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
...
function various_payment_prepare_head($object)
{
    ...
    $head[$h][0] = constant('BASE_URL') . '/compta/bank/various_payment/card.php?id=' . $object->id;
    ...
    $nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
    $nbLinks = Link::count($db, $object->element, $object->id);
    $head[$h][0] = constant('BASE_URL') . '/compta/bank/various_payment/document.php?id=' . $object->id;
    $head[$h][1] = $langs->trans('Documents');
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
    ...
    $head[$h][0] = constant('BASE_URL') . '/compta/bank/various_payment/info.php?id=' . $object->id;
    $head[$h][1] = $langs->trans("Info");
    $head[$h][2] = 'info';
    $h++;
