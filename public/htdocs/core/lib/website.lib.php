...
function dolWebsiteReplacementOfLinks($website, $content, $removephppart = 0, $contenttype = 'html', $containerid = 0)
{
    ...
    $content = preg_replace('/(href=")\/?([^:\"\!]*)\.php(#[^\"<>]*)?\"/', '\1!~!~!~' . constant('BASE_URL') . '/website/index.php?website=' . $website->ref . '&pageref=\2\3"', $content, -1, $nbrep);
    // Replace relative link /xxx.php?a=b&c=d#aaa or /xxx.php?a=b&c=d with dolibarr URL
    $content = preg_replace('/(href=")\/?([^:\"\!]*)\.php\?([^#\"<>]*)(#[^\"<>]*)?\"/', '\1!~!~!~' . constant('BASE_URL') . '/website/index.php?website=' . $website->ref . '&pageref=\2&\3\4"', $content, -1, $nbrep);
    ...
    $content = preg_replace('/data-slide-bg=(["\']?)\/?medias\//', 'data-slide-bg=\1!~!~!~' . constant('BASE_URL') . '/viewimage.php?modulepart=medias&file=', $content, -1, $nbrep);
    ...
    $content = preg_replace('/(<img[^>]*src=")\/?medias\//', '\1!~!~!~' . constant('BASE_URL') . '/viewimage.php?modulepart=medias&file=', $content, -1, $nbrep);
    // <img src="image.png... => <img src="dolibarr/viewimage.php/modulepart=medias&file=image.png...
    $content = preg_replace('/(<img[^>]*src=")\/?([^:\"\!]+)\"/', '\1!~!~!~' . constant('BASE_URL') . '/viewimage.php?modulepart=medias&file=\2"', $content, -1, $nbrep);
    // <img src="viewimage.php/modulepart=medias&file=image.png" => <img src="dolibarr/viewimage.php/modulepart=medias&file=image.png"
    $content = preg_replace('/(<img[^>]*src=")(\/?viewimage\.php)/', '\1!~!~!~' . constant('BASE_URL') . '/viewimage.php', $content, -1, $nbrep);
    ...
    $content = str_replace('href="!~!~!~/document.php', 'href="!~!~!~' . constant('BASE_URL') . '/document.php', $content);
...
function dolWebsiteOutput($content, $contenttype = 'html', $containerid = 0)
{
    ...
    if (defined('USEDOLIBARREDITOR')) {     // REPLACEMENT OF LINKS When page called from Dolibarr editor
        ...
        $content = preg_replace('/(href=")\/?([^:\"\!]*)\.php(#[^\"<>]*)?\"/', '\1!~!~!~' . constant('BASE_URL') . '/public/website/index.php?website=' . $website->ref . '&pageref=\2\3"', $content, -1, $nbrep);
        // Replace relative link /xxx.php?a=b&c=d#aaa or /xxx.php?a=b&c=d with dolibarr URL
        ...
        $content = preg_replace('/(href=")\/?([^:\"\!]*)\.php\?([^#\"<>]*)(#[^\"<>]*)?\"/', '\1!~!~!~' . constant('BASE_URL') . '/public/website/index.php?website=' . $website->ref . '&pageref=\2&\3\4"', $content, -1, $nbrep);
        // Replace occurrence like _service_XXX.php with dolibarr URL
        $content = preg_replace('/([\'"])_service_([^\'"]+)\.php\1/', '\1!~!~!~' . constant('BASE_URL') . '/public/website/index.php?website=' . $website->ref . '&pageref=_service_\2\1', $content, -1, $nbrep);
        // Replace occurrence like _library_XXX.php with dolibarr URL
        $content = preg_replace('/([\'"])_library_([^\'"]+)\.php\1/', '\1!~!~!~' . constant('BASE_URL') . '/public/website/index.php?website=' . $website->ref . '&pageref=_library_\2\1', $content, -1, $nbrep);
        ...
        // action="newpage.php" => action="dolibarr/website/index.php?website=...&pageref=newpage
        $content = preg_replace('/(action=")\/?([^:\"]*)(\.php\")/', '\1!~!~!~' . constant('BASE_URL') . '/public/website/index.php?website=' . $website->ref . '&pageref=\2"', $content, -1, $nbrep);
...
function getImagePublicURLOfObject($object, $no = 1, $extName = '')
{
    ...
    if ($resql) {
            ...
            if ($obj) {
                if (empty($obj->share)) {
                    ...
                    if (defined('USEDOLIBARRSERVER') || defined('USEDOLIBARREDITOR')) {
                        $image_path = constant('BASE_URL') . '/viewimage.php?hashp=' . urlencode($obj->share);
                    } else {
        ...
        if (!$found && $foundnotshared) {
            if (defined('USEDOLIBARRSERVER') || defined('USEDOLIBARREDITOR')) {
                $image_path = constant('BASE_URL') . '/viewimage.php?modulepart=common&file=nophotopublic.png';
            } else {
    ...
    if (empty($image_path)) {
        if (defined('USEDOLIBARRSERVER') || defined('USEDOLIBARREDITOR')) {
            $image_path = constant('BASE_URL') . '/viewimage.php?modulepart=common&file=nophoto.png';
        } else {
...
function getAllImages($object, $objectpage, $urltograb, &$tmp, &$action, $modifylinks = 0, $grabimages = 1, $grabimagesinto = 'subpage')
{
    ...
    foreach ($regs[0] as $key => $val) {
        ...
        if ($modifylinks) {
            $tmp = preg_replace('/' . preg_quote($regs[0][$key], '/') . '/i', '<img' . $regs[1][$key] . 'src="' . constant('BASE_URL') . '/viewimage.php?modulepart=medias&file=' . $filename . '"' . $regs[3][$key] . '>', $tmp);
        }
    }
    ...
    foreach ($regs[0] as $key => $val) {
        ...
        if ($modifylinks) {
            $tmp = preg_replace('/' . preg_quote($regs[0][$key], '/') . '/i', 'background' . $regs[1][$key] . 'url("' . constant('BASE_URL') . '/viewimage.php?modulepart=medias&file=' . $filename . '")', $tmp);
        }
    }
}
