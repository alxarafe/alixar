...
class FormBarCode
{
    ...
    public function setBarcodeEncoder($selected, $barcodelist, $code_id, $idForm = 'formbarcode')
    {
        ...
        if (
            ((isModEnabled("product") || isModEnabled("service")) && getDolGlobalString('PRODUIT_DEFAULT_BARCODE_TYPE') == $code_id) ||
        ...
        if (!empty($conf->use_javascript_ajax)) {
            $select_encoder = '<form action="' . constant('BASE_URL') . '/admin/barcode.php" method="POST" id="form' . $idForm . '">';
