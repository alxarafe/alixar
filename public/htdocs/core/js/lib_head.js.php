...
function setConstant(url, code, input, entity, strict, forcereload, userid, token, value) {
    ...
    $.post(url, {
        ...
        console.log("Error, we force reload");
        location.reload();
    }); /* When it fails, we always force reload to have setEventErrorMessages in session visible */
    ...
    function show_preview(mode) {
        ...
        if (mode == 'image' && showOriginalSizeButton) {
            ...
            optionsbuttons = {
                ...
                "<?php echo dol_escape_js($langs->transnoentitiesnoconv("RotateImage")); ?>": function () {
                    curRot += 90;
                    jQuery(".ui-dialog-content.ui-widget-content > object").css("transform", "rotate(" + curRot + "deg)");
                },
        ...
        if (showOriginalSizeButton) {
            jQuery(".ui-dialog-content.ui-widget-content > object").css({
                "max-height": "100%",
                "width": "auto",
                "margin-left": "auto",
                "margin-right": "auto",
...
function dolroundjs(number, decimals) {
...
function pricejs(amount, mode = 'MT', currency_code = '', force_locale = '') {
    ...
    if (currencyCache[currency_code]
        ...
        currency_symbol = currencyCache[currency_code]['unicode'].reduce(function (res, cur) {
            return res + cur
        }, '');
    }
