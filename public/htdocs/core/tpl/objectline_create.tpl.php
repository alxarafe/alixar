...
if ($nolinesbefore) {
    ...
    <tr class="liste_titre<?php echo(($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_add_') ?> nodrag nodrop">
        ...
        <td class="linecoldescription minwidth400imp">
            <div id="add"></div>
            <span class="hideonsmartphone"><?php echo $langs->trans('AddNewLine'); ?></span>
        </td>
        <?php
        if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $object->element == 'invoice_supplier_rec') {  // We must have same test in printObjectLines
            ?>
            <td class="linecolrefsupplier"><span id="title_fourn_ref"><?php echo $langs->trans('SupplierRef'); ?></span>
            </td>
            <?php
        } ?>
...
?>
    <tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
        ...
        <td class="nobottom linecoldescription minwidth400imp">
            ...
            if ($addproducton || $addserviceon) {
                if ($addproducton && $addserviceon) {
                    ...
                    echo '<a class="dropdown-item" href="' . constant('BASE_URL') . '/product/card.php?action=create&type=0&backtopage=' . $_SERVER["PHP_SELF"] . ('?id=' . $object->id) . '"> ' . $langs->trans("NewProduct") . '</a>';
                    echo '<a class="dropdown-item" href="' . constant('BASE_URL') . '/product/card.php?action=create&type=1&backtopage=' . $_SERVER["PHP_SELF"] . ('?id=' . $object->id) . '"> ' . $langs->trans("NewService") . '</a>';
                    echo '</div>';
                    echo '</div>';
                } else {
                    if ($addproducton) {
                        $url = '/product/card.php?leftmenu=product&action=create&type=0&backtopage=' . $_SERVER["PHP_SELF"];
                        ...
                        if (getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2) {
                            ...
                            print '<a href="' . constant('BASE_URL') . '/product/card.php?action=create&type=0&backtopage=' . $_SERVER["PHP_SELF"] . ('?id=' . $object->id) . '" title="' . dol_escape_htmltag($langs->trans("NewProduct")) . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                        }
                    }
                    if ($addserviceon) {
                        $url = '/product/card.php?leftmenu=product&action=create&type=1&backtopage=' . $_SERVER["PHP_SELF"];
                        ...
                        if (getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2) {
                            ...
                            print '<a href="' . constant('BASE_URL') . '/product/card.php?action=create&type=1&backtopage=' . $_SERVER["PHP_SELF"] . ('?id=' . $object->id) . '" title="' . dol_escape_htmltag($langs->trans("NewService")) . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                        }
                    }
                }
            } ?>
        ...
        <td class="nobottom linecolrefsupplier"><input id="fourn_ref" name="fourn_ref"
                                                       ...
                                                       value="<?php echo(GETPOSTISSET("fourn_ref") ? GETPOST("fourn_ref", 'alpha', 2) : ''); ?>">
        </td>
    <?php
    }
    ...
    ?>
        ...
        <td class="nobottom linecoluht right"><?php $coldisplay++; ?>
            <input type="text" name="price_ht" id="price_ht" class="flat right width50"
        ...
        if (isModEnabled("multicurrency") && $this->multicurrency_code != $conf->currency) {
            ...
            <td class="nobottom linecoluht_currency right">
                <input type="text" name="multicurrency_price_ht" id="multicurrency_price_ht" class="flat right width50"
        ...
        if (!empty($inputalsopricewithtax) && !getDolGlobalInt('MAIN_NO_INPUT_PRICE_WITH_TAX')) {
            ...
            <td class="nobottom linecoluttc right">
                <input type="text" name="price_ttc" id="price_ttc" class="flat right width50"
        ...
        <td class="nobottom linecolqty right">
            ...
            <input type="text" name="qty" id="qty" class="flat width40 right"
                   value="<?php echo(GETPOSTISSET("qty") ? GETPOST("qty", 'alpha', 2) : $default_qty); ?>">
        </td>
        <?php
        if (getDolGlobalInt('PRODUCT_USE_UNITS')) {
            $coldisplay++;
...
if (!empty($usemargins) && $user->hasRight('margins', 'creer')) {
    ...
    options += '
    <option value="'+this.id+'" price="'+this.price+'">'+this.label+'</option>';
    });
    options += '
    <option value="inputprice"
            price="'+defaultprice+'"><?php echo dol_escape_js($langs->trans("InputPrice") . '...'); ?></option>';
