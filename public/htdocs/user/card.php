...
if ($action == 'create' || $action == 'adduserldap') {
    ...
    if (getDolGlobalInt('MAIN_MULTILANGS')) {
        ...
        print img_picto('', 'language', 'class="pictofixedwidth"') . FormAdmin::selectLanguage(GETPOST('default_lang', 'alpha') ? GETPOST('default_lang', 'alpha') : ($object->lang ? $object->lang : ''), 'default_lang', 0, 0, 1, 0, 0, 'maxwidth300 widthcentpercentminusx');
        print '</td>';
        print '</tr>';
    }
    ...
    if ($id > 0) {
        ...
        if ($mode == 'employee') { // For HRM module development
            ...
            $linkback = '<a href="' . constant('BASE_URL') . '/hrm/employee/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
        } else {
        ...
        if ($action != 'edit') {
            ...
            $morehtmlref = '<a href="' . constant('BASE_URL') . '/user/vcard.php?id=' . $object->id . '&output=file&file=' . urlencode(dol_sanitizeFileName($object->getFullName($langs) . '.vcf')) . '" class="refid" rel="noopener" rel="noopener">';
        ...
        if ($action == 'edit' && ($canedituser || $caneditpasswordandsee)) {
            ...
            if (isModEnabled('stock') && getDolGlobalString('MAIN_DEFAULT_WAREHOUSE_USER')) {
                ...
                print $formproduct->selectWarehouses($object->fk_warehouse, 'fk_warehouse', 'warehouseopen', 1);
                print ' <a href="' . constant('BASE_URL') . '/product/stock/card.php?action=create&token=' . newToken() . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id . '&action=edit&token=' . newToken()) . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddWarehouse") . '"></span></a>';
                print '</td></tr>';
            }
            ...
            if (getDolGlobalInt('MAIN_MULTILANGS')) {
                ...
                print img_picto('', 'language', 'class="pictofixedwidth"') . FormAdmin::selectLanguage($object->lang, 'default_lang', 0, null, '1', 0, 0, 'widthcentpercentminusx maxwidth300');
                print '</td>';
                print '</tr>';
            }
