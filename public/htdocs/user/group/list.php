...
ViewMain::llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'bodyforlist mod-user page-group_list');
...
if ($caneditperms) {
    ...
    $newcardbutton .= dolGetButtonTitle($langs->trans('NewGroup'), '', 'fa fa-plus-circle', constant('BASE_URL') . '/user/group/card.php?action=create&leftmenu=', '', $caneditperms);
}
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print '<a href="' . constant('BASE_URL') . '/user/group/perms.php?id=' . $obj->rowid . '">' . dol_escape_htmltag($obj->nbpermissions) . '</a>';
        print '</td>';
        if (!$i) {
            $totalarray['nbfield']++;
        }
