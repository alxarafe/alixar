    ...
    if (count($objimport->array_import_module)) {
        ...
        foreach ($sortedarrayofmodules as $key => $value) {
            ...
            if ($objimport->array_import_perms[$key]) {
                print '<a href="' . constant('BASE_URL') . '/imports/import.php?step=2&datatoimport=' . $objimport->array_import_code[$key] . $param . '">' . img_picto($langs->trans("NewImport"), 'next', 'class="fa-15"') . '</a>';
            } else {
...
if ($step == 2 && $datatoimport) {
    ...
    foreach ($list as $key) {
        ...
        if (empty($objmodelimport->drivererror[$key])) {
            ...
            print '<a href="' . constant('BASE_URL') . '/imports/emptyexample.php?format=' . $key . $param . '&output=file&file=' . urlencode($filename) . '" target="_blank" rel="noopener noreferrer">';
        ...
        if (empty($objmodelimport->drivererror[$key])) {
            print '<a href="' . constant('BASE_URL') . '/imports/import.php?step=3&format=' . $key . $param . '">' . img_picto($langs->trans("SelectFormat"), 'next', 'class="fa-15"') . '</a>';
        }
        print '</td>';
        print '</tr>';
    }
...
if ($step == 3 && $datatoimport) {
    ...
    print '<a href="' . constant('BASE_URL') . '/imports/emptyexample.php?format=' . $format . $param . '&output=file&file=' . urlencode($filename) . '" target="_blank" rel="noopener noreferrer">';
    print img_picto('', 'download', 'class="paddingright opacitymedium"');
    print $langs->trans("DownloadEmptyExampleShort");
    print '</a>';
    print $form->textwithpicto('', $langs->trans("DownloadEmptyExample") . '.<br>' . $langs->trans("StarAreMandatory"));
    ...
    if (count($filearray) > 0) {
        ...
        foreach ($filearray as $key => $val) {
            ...
            print '<a data-ajax="false" href="' . constant('BASE_URL') . '/document.php?modulepart=' . $modulepart . '&file=' . urlencode($relativepath) . '&step=3' . $param . '" target="_blank" rel="noopener noreferrer">';
            print $file;
            print '</a>';
            print '</td>';
...
if ($step == 4 && $datatoimport) {
    ...
    print '<a data-ajax="false" href="' . constant('BASE_URL') . '/document.php?modulepart=' . $modulepart . '&file=' . urlencode($relativepath) . '&step=4' . $param . '" target="_blank" rel="noopener noreferrer">';
    print img_mime($file, '', 'pictofixedwidth');
    print $filetoimport;
...
if ($step == 5 && $datatoimport) {
    ...
    if ($action != 'launchsimu') {
        ...
        if ($user->hasRight('import', 'run')) {
            if (empty($nboferrors)) {
                print '<a class="butAction" href="' . constant('BASE_URL') . '/imports/import.php?leftmenu=import&step=6&importid=' . $importid . $param . '">' . $langs->trans("RunImportFile") . '</a>';
            } else {
