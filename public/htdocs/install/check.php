...
print '<h3><img class="valignmiddle inline-block paddingright" src="' . constant('BASE_URL') . '/htdocs/theme/common/octicons/build/svg/gear.svg" width="20" alt="Database"> ';
...
if (!empty($useragent)) {
	...
	if ($browsername == 'ie' && $browserversion < 7) {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("WarningBrowserTooOld")."<br>\n";
	}
}
...
if (versioncompare(versionphparray(), $arrayphpminversionerror) < 0) {        // Minimum to use (error if lower)
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionerror));
	...
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionwarning));
	$checksok = 1; // 0=error, 1=warning
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPVersion")." ".versiontostring(versionphparray());
}
...
if (versioncompare(versionphparray(), $arrayphpmaxversionwarning) > 0 && versioncompare(versionphparray(), $arrayphpmaxversionwarning) < 3) {        // Maximum to use (warning if higher)
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPVersionTooHigh", versiontostring($arrayphpmaxversionwarning));
	$checksok = 1; // 0=error, 1=warning
	print "<br>\n";
}
...
if (!isset($_GET["testget"]) && !isset($_POST["testpost"])) {	// We must keep $_GET and $_POST here
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Warning" class="valignmiddle"> '.$langs->trans("PHPSupportPOSTGETKo");
	print ' (<a href="'.dol_escape_htmltag($_SERVER["PHP_SELF"]).'?testget=ok">'.$langs->trans("Recheck").'</a>)';
	print "<br>\n";
	$checksok = 0;
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupportPOSTGETOk")."<br>\n";
}


// Check if session_id is enabled
if (!function_exists("session_id")) {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupportSessions")."<br>\n";
	$checksok = 0;
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupportSessions")."<br>\n";
}
...
if (!extension_loaded("mbstring")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "MBString")."<br>\n";
	// $checksok = 0; // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "MBString")."<br>\n";
}

// Check for json extension
if (!extension_loaded("json")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "JSON")."<br>\n";
	// $checksok = 0; // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "JSON")."<br>\n";
}

// Check if GD is supported (we need GD for image conversion)
if (!function_exists("imagecreate")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "GD")."<br>\n";
	// $checksok = 0;       // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "GD")."<br>\n";
}

// Check if Curl is supported
if (!function_exists("curl_init")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "Curl")."<br>\n";
	// $checksok = 0;       // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "Curl")."<br>\n";
}

// Check if PHP calendar extension is available
if (!function_exists("easter_date")) {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "Calendar")."<br>\n";
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "Calendar")."<br>\n";
}

// Check if Xml is supported
if (!function_exists("simplexml_load_string")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "Xml")."<br>\n";
	// $checksok = 0;       // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "Xml")."<br>\n";
}

// Check if UTF8 is supported
if (!function_exists("utf8_encode")) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "UTF8")."<br>\n";
	// $checksok = 0; // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "UTF8")."<br>\n";
}
...
if (empty($_SERVER["SERVER_ADMIN"]) || $_SERVER["SERVER_ADMIN"] != 'doliwamp@localhost') {
	if (!function_exists("locale_get_primary_language") || !function_exists("locale_get_region")) {
		$langs->load("errors");
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "Intl")."<br>\n";
		// $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
	} else {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "Intl")."<br>\n";
	}
}
...
if (PHP_VERSION_ID <= 80300) {
	if (!function_exists("imap_open")) {
		$langs->load("errors");
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "IMAP")."<br>\n";
		// $checksok = 0;       // If ko, just warning. So check must still be 1 (otherwise no way to install)
	} else {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "IMAP")."<br>\n";
	}
}
...
if (!class_exists('ZipArchive')) {
	$langs->load("errors");
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> '.$langs->trans("ErrorPHPDoesNotSupport", "ZIP")."<br>\n";
	// $checksok = 0;       // If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPSupport", "ZIP")."<br>\n";
}
...
if ($memmaxorig != '') {
	...
	if ($memmax >= $memrequired || $memmax == -1) {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> '.$langs->trans("PHPMemoryOK", $memmaxorig, $memrequiredorig)."<br>\n";
	} else {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/warning.png" alt="Warning" class="valignmiddle"> '.$langs->trans("PHPMemoryTooLow", $memmaxorig, $memrequiredorig)."<br>\n";
	}
}
...
if (!file_exists($conffile)) {
	print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> '.$langs->trans("ConfFileDoesNotExistsAndCouldNotBeCreated", $conffiletoshow);
	...
	if (dol_is_dir($conffile)) {
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/error.png" alt="Warning"> '.$langs->trans("ConfFileMustBeAFileNotADir", $conffiletoshow);

		$allowinstall = 0;
	} elseif (!is_writable($conffile)) {
		// File exists but cannot be modified
		if ($confexists) {
			print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok"> '.$langs->trans("ConfFileExists", $conffiletoshow);
		} else {
			print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok"> '.$langs->trans("ConfFileCouldBeCreated", $conffiletoshow);
		}
		print "<br>";
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Warning"> '.$langs->trans("ConfFileIsNotWritable", $conffiletoshow);
		print "<br>\n";
		...
		print "<br>";
		print '<img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok"> '.$langs->trans("ConfFileIsWritable", $conffiletoshow);
		print "<br>\n";
	...
	if ($checksok) {
		...
		if (empty($dolibarr_main_db_host)) {	// This means install process was not run
			...
			// <img src="' . constant('BASE_URL') . '/htdocs/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ';
		}
