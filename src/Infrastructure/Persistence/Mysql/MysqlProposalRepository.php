...
use App\Domain\Proposal\Proposal;
use App\Domain\Proposal\ProposalRepository;
...
class MysqlProposalRepository implements ProposalRepository
{
    ...
    public function updateLine(int $proposalId, int $lineId, array $data): void
    {
        ...
        $sql = sprintf("UPDATE {$this->table}det SET %s WHERE rowid = :lineId AND fk_propal = :propId", implode(', ', $sets));
