...
class MysqlInvoiceRepository implements InvoiceRepository
{
        ...
        try {
            ...
            foreach ($invoice->getLines() as $line) {
                if ($line->getInvoiceId() === 0 || $line->getInvoiceId() === null) {
                    $lineReflection = new \ReflectionClass(\App\Domain\Invoice\InvoiceLine::class);
    ...
    {
        $stmt = $this->pdo->prepare(
            'DELETE FROM ' . $this->tableLines . ' WHERE rowid = :id'
        );
    ...
    public function updateTotals(int $invoiceId): void
    {
        ...
        // Update DB header
        $stmt = $this->pdo->prepare(
            'UPDATE ' . $this->table . ' SET total_ht = :ht, total_tva = :tva, total_ttc = :ttc WHERE rowid = :id'
        );
        $stmt->execute([
