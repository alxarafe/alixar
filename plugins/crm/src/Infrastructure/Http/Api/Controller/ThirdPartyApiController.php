...
use Plugin\Crm\Domain\ThirdParty\ThirdPartyNotFoundException;
...
class ThirdPartyApiController
{
    ...
    public function __construct(
        ...
        private \Plugin\Crm\Domain\ThirdParty\ThirdPartyValidator $validator,
        private ?\Plugin\Crm\Domain\ThirdParty\BankAccountRepository $bankAccountRepository = null,
        private ?\Plugin\Crm\Domain\Category\ThirdPartyCategoryRepository $categoryRepository = null,
        private ?\Plugin\Crm\Domain\User\ThirdPartyRepresentativeRepository $representativeRepository = null,
    ) {
    }
    ...
    public function create(): void
    {
        ...
        $tp = new ThirdParty(
            ...
            type: \Plugin\Crm\Domain\ThirdParty\ThirdPartyType::from((int) ($cleanData['type'] ?? 0)),
            isSupplier: (bool) ($cleanData['isSupplier'] ?? false),
            status: isset($cleanData['status'])
                ? \Plugin\Crm\Domain\ThirdParty\ThirdPartyStatus::from((int) $cleanData['status'])
                : null,
    ...
    public function getBankAccounts(int $id): void
    {
        ...
        Flight::json(array_map(fn($acc) => $this->mapToDolibarr(
            $acc->toArray(),
            \Plugin\Crm\Infrastructure\Persistence\Mysql\MysqlBankAccountRepository::COLUMN_MAP
        ), $accounts));
    }
    ...
    public function postBankAccount(int $id): void
    {
        ...
        $payload = json_decode((string) file_get_contents('php://input'), true) ?? [];
        $cleanData = $this->mapToClean($payload, \Plugin\Crm\Infrastructure\Persistence\Mysql\MysqlBankAccountRepository::COLUMN_MAP);

        $account = new \Plugin\Crm\Domain\ThirdParty\BankAccount(thirdPartyId: $id);
    ...
    public function putBankAccount(int $id, int $bankaccountId): void
    {
        ...
        Flight::json($this->mapToDolibarr($account->toArray(), \Plugin\Crm\Infrastructure\Persistence\Mysql\MysqlBankAccountRepository::COLUMN_MAP));
    }
    ...
    public function getCategories(int $id): void
    {
        ...
        Flight::json(array_map(fn(\Core\Domain\Category\Category $cat) => $this->mapToDolibarr(
            $cat->toArray(),
            \Plugin\Crm\Infrastructure\Persistence\Mysql\Category\MysqlThirdPartyCategoryRepository::CATEGORY_COLUMN_MAP
        ), $categories));
    }
    ...
    public function getRepresentatives(int $id): void
    {
        ...
        Flight::json(array_map(fn(\Core\Domain\User\User $user) => $user->toArray(), $users));
    }
