...
/**
 * DolibarrMysqlEventRepository — PDO adapter for the Dolibarr `actioncomm` table.
 *
 * This implements the pure EventRepository from the CRM domain but reads/writes
 * physically to the legacy `actioncomm` table in Dolibarr's database.
 *
 * Part of the Strangler Fig pattern: the CRM plugin knows nothing about
 * Dolibarr's schema; all the dirty mapping lives here in quarantine.
 */
class DolibarrMysqlEventRepository implements EventRepository
{
    use DolibarrMappingTrait;

    private string $table;

    // Dolibarr's 'llx_actioncomm' database schema mapping
    ...
    public function save(Event $event): void
    {
        ...
        if ($event->getId() === null) {
            ...
            if (!isset($dbData['fk_parent'])) {
                $dbData['fk_parent'] = 0;
            }
            if (!isset($dbData['datec'])) {
                $dbData['datec'] = date('Y-m-d H:i:s');
            }
            if (empty($dbData['percent'])) {
                $dbData['percent'] = 0;
            }
            if (empty($dbData['fk_user_author'])) {
                $dbData['fk_user_author'] = 1; // Fallback to superadmin if missing
            }

            // Fix required fields
            if (empty($dbData['ref'])) {
                $dbData['ref'] = 'AC' . date('YmdHis'); // Auto-ref if not provided
            }
            if (empty($dbData['label'])) {
                $dbData['label'] = 'New Event';
            }
