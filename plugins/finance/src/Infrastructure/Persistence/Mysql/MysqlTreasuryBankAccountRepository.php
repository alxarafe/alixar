...
namespace Plugin\Finance\Infrastructure\Persistence\Mysql;
...
class MysqlTreasuryBankAccountRepository implements BankAccountRepository
{
    ...
    public function updateTransaction(int $accountId, int $transactionId, array $data): void
    {
        ...
        $sql = sprintf("UPDATE {$prefix} SET %s WHERE id = :transactionId AND fk_account = :accountId", implode(', ', $sets));
    ...
    {
        $prefix = str_replace('bank_account', 'bank', $this->table); // Get llx_bank
        $stmt = $this->pdo->prepare("DELETE FROM {$prefix} WHERE id = :transactionId AND fk_account = :accountId");
