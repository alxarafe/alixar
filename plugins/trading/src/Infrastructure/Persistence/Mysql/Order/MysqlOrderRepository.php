...
namespace Plugin\Trading\Infrastructure\Persistence\Mysql\Order;
...
class MysqlOrderRepository extends AbstractMysqlTradingRepository implements OrderRepository
{
    private string $table = 'orders';
    private string $lineTable = 'order_lines';
    private string $contactTable = 'order_contacts';
    private string $fkColumn = 'order_id';
    ...
    {
        return $this->getContactsBase($orderId, $this->contactTable, $this->fkColumn, $type);
    }
    public function addContact(int $orderId, int $contactId, string $type): void
    {
        $this->addContactBase($orderId, $contactId, $type, $this->contactTable, $this->fkColumn);
    }
    public function deleteContact(int $orderId, int $contactId, string $type): void
    {
        $this->deleteContactBase($orderId, $contactId, $type, $this->contactTable, $this->fkColumn);
    }
    ...
    private function mapRow(array $row): Order
    {
        ...
        return Order::fromArray($data + ['id' => $row['id']]);
    }
}
