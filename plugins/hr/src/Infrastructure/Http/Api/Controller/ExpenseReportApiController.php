...
namespace Plugin\Hr\Infrastructure\Http\Api\Controller;
...
class ExpenseReportApiController
{
    ...
    public function show(string $id): void
    {
        ...
        if (!$item) {
            \Flight::json(['error' => 'Expense report not found'], 404);
            return;
        }
    ...
    public function create(): void
    {
        ...
        if (!isset($data['userId'])) {
            throw new DomainValidationException(['userId' => 'Missing required fields: userId'], 'Invalid expense report data');
        }

        $item = ExpenseReport::fromArray($data);
    ...
    public function update(string $id): void
    {
        ...
        $data = \Flight::request()->data->getData();
        $updatedItem = ExpenseReport::fromArray(array_merge($item->toArray(), $data));
